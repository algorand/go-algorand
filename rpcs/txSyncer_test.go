...
func (mock mockPendingTxAggregate) Pending() (pendingTxn [][]transactions.SignedTxn, pendingTxnCount int) {
	return bookkeeping.SignedTxnsToGroups(mock.txns), len(mock.txns)
}
...
func TestSyncFromClient(t *testing.T) {
	...
	pendingTxGroups, _ := serverPool.Pending()
	runner := MockRunner{failWithNil: false, failWithError: false, txgroups: pendingTxGroups[len(pendingTxGroups)-1:], done: make(chan *rpc.Call)}
...
func TestSyncFromUnsupportedClient(t *testing.T) {
	...
	pendingTxGroups, _ := pool.Pending()
	runner := MockRunner{failWithNil: true, failWithError: false, txgroups: pendingTxGroups[len(pendingTxGroups)-1:], done: make(chan *rpc.Call)}
	client := MockRPCClient{client: &runner, log: logging.TestingLog(t)}
	clientAgg := MockClientAggregator{peers: []network.Peer{&client}}
	handler := mockHandler{}
...
func TestSyncFromClientAndError(t *testing.T) {
	...
	pool := makeMockPendingTxAggregate(3)
	pendingTxGroups, _ := pool.Pending()
	runner := MockRunner{failWithNil: false, failWithError: true, txgroups: pendingTxGroups[len(pendingTxGroups)-1:], done: make(chan *rpc.Call)}
	client := MockRPCClient{client: &runner, log: logging.TestingLog(t)}
	clientAgg := MockClientAggregator{peers: []network.Peer{&client}}
	handler := mockHandler{}
	syncer := MakeTxSyncer(pool, &clientAgg, &handler, testSyncInterval, testSyncTimeout, config.GetDefaultLocal().TxSyncServeResponseSize)
