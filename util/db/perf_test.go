...
func BenchmarkSQLWrites(b *testing.B) {
	...
	err = wdb.Atomic("BenchmarkSQLWrites_CreateT", func(tx *sql.Tx) error {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		err = wdb.Atomic("BenchmarkSQLWrites_InsertT", func(tx *sql.Tx) error {
...
func BenchmarkSQLErasableWrites(b *testing.B) {
	...
	err = wdb.Atomic("BenchmarkSQLErasableWrites_CreateT", func(tx *sql.Tx) error {
		_, err := tx.Exec("CREATE TABLE t (a integer primary key, b integer)")
		if err != nil {
			return err
		}
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		err = wdb.Atomic("BenchmarkSQLErasableWrites_InsertT", func(tx *sql.Tx) error {
			_, err := tx.Exec("INSERT INTO t (a, b) VALUES (?, ?)", i, i)
			return err
		})
		require.NoError(b, err)
...
func BenchmarkSQLQueryAPIs(b *testing.B) {
	...
	err = wdb.Atomic("BenchmarkSQLQueryAPIs_CreateT", func(tx *sql.Tx) error {
		_, err := tx.Exec("CREATE TABLE t (a integer primary key, b integer)")
		if err != nil {
			return err
		}
	...
	b.Run("rdb.Atomic", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			err = rdb.Atomic("BenchmarkSQLQueryAPIs_SelectT", func(tx *sql.Tx) error {
				var b int
	...
	b.Run("wdb.Atomic", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			err = wdb.Atomic("BenchmarkSQLQueryAPIs_SelectT", func(tx *sql.Tx) error {
				var r int
