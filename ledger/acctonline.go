...
func (ao *onlineAccounts) TopOnlineAccounts(rnd basics.Round, voteRnd basics.Round, n uint64, params config.ConsensusParams, hdr bookkeeping.BlockHeader) (topOnlineAccounts []*ledgercore.OnlineAccount, totalOnlineStake basics.MicroAlgos, err error) {
	...
	for {
		...
		for _, oa := range invalidOnlineAccounts {
			...
			if params.StateProofExcludeTotalWeightWithRewards {
				rewards := basics.PendingRewards(&ot, params, oa.MicroAlgos, oa.RewardsBase, hdr.RewardsLevel)
