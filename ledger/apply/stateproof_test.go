...
func TestApplyStateProof(t *testing.T) {
	...
	err = StateProof(stateProofTx, atRound, applier, validate)
	a.ErrorContains(err, `the number of reveals is not large enough to prove that the desired weight signed, with the desired security level: state proof crypto error`)
	a.Contains(err.Error(), "crypto error")
