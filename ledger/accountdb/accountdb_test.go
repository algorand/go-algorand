...
func TestAccountDBInit(t *testing.T) {
	...
	newDB, err := AccountsInitTest(tx, accts, protocol.ConsensusCurrentVersion)
	require.NoError(t, err)
...
func TestAccountDBRound(t *testing.T) {
	...
	accts := ledgertesting.RandomAccounts(20, true)
	_, err = AccountsInitTest(tx, accts, protocol.ConsensusCurrentVersion)
	require.NoError(t, err)
	checkAccounts(t, tx, 0, accts)
	totals, err := AccountsTotals(context.Background(), tx, false)
	require.NoError(t, err)
	...
	for i := 1; i < 10; i++ {
		...
		require.NoError(t, err)
		require.Equal(t, updatesCnt.Len(), updatedAccts.Count)
		numResUpdates := 0
...
func TestAccountDBInMemoryAcct(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(fmt.Sprintf("test%d", i), func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, 1, updatedAccts.Count) // we store empty even for deleted accounts
			require.Equal(t,
...
func benchmarkInitBalances(b testing.TB, numAccounts int, dbs db.Pair, proto protocol.ConsensusVersion) (updates map[basics.Address]basics.AccountData) {
	...
	_, err = AccountsInitTest(tx, updates, proto)
	require.NoError(b, err)
	err = tx.Commit()
	require.NoError(b, err)
	return
...
func TestAccountsReencoding(t *testing.T) {
	...
	err := dbs.Wdb.Atomic(func(ctx context.Context, tx *sql.Tx) (err error) {
		_, err = AccountsInitTest(tx, make(map[basics.Address]basics.AccountData), protocol.ConsensusCurrentVersion)
		if err != nil {
			return err
		}
...
func TestCompactDeltas(t *testing.T) {
	...
	baseAccounts.Init(nil, 100, 80)
	outAccountDeltas := MakeCompactAccountDeltas(accountDeltas, basics.Round(1), true, baseAccounts)
	outCreatableDeltas := ledgercore.CompactCreatableDeltas(creatableDeltas)
...
func TestAccountOnlineQueries(t *testing.T) {
	...
	require.Equal(t, 2, len(poads))
	require.Equal(t, int64(1), poads[0].ID())
	require.Equal(t, basics.Round(1), poads[0].UpdRound())
	require.Equal(t, int64(3), poads[1].ID())
	...
	require.Equal(t, int64(5), poads[0].ID())
