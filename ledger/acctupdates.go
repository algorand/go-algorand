...
func (au *accountUpdates) initialize(cfg config.Local) {
	...
	case forceCatchpointFileGenerationTrackingMode:
		// this CatchpointTracking is used for debugging of the catchpoint generation.
		fallthrough
	case 2:
		au.enableGeneratingCatchpointFiles = true
	default:
		au.enableGeneratingCatchpointFiles = cfg.Archival
	}
...
func (au *accountUpdates) lookupLatest(addr basics.Address) (data basics.AccountData, rnd basics.Round, err error) {
	...
	for {
		...
		if resourceDbRound < currentDbRound {
			...
			return basics.AccountData{}, basics.Round(0), &StaleDatabaseRoundError{databaseRound: resourceDbRound, memoryRound: currentDbRound}
		}
...
func (au *accountUpdates) prepareCommit(dcc *deferredCommitContext) error {
	...
	if au.versions[1] != au.versions[offset] {
		...
		if dcc.isCatchpointRound && au.enableGeneratingCatchpointFiles {
