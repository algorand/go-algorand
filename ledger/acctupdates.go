...
func (au *accountUpdates) committedUpTo(rnd basics.Round) basics.Round {
	...
	err := au.dbs.wdb.Atomic(func(tx *sql.Tx) error {
		...
		for i := uint64(0); i < offset; i++ {
			err := accountsNewRound(tx, au.deltas[i], au.roundTotals[i+1].RewardsLevel, au.protos[i+1])
			if err != nil {
				return err
			}
		...
		return updateAccountsRound(tx, au.dbRound+basics.Round(offset), 0)
	})
	if err != nil {
