...
func TestMinBalanceErrorMessage(t *testing.T) {
	genesisInitState, addrs, keys := genesis(10)
	...
	// TODO evaluate and apply an I-accept-assets transaction or otherwise find a way to change addrs[0]'s state

	txnGoesBelowMinBal := transactions.Transaction{
		Type: protocol.PaymentTx,
		Header: transactions.Header{
			Sender:      addrs[0],
			Fee:         minFee,
	...
	// evaluating transaction that goes below should min bal
	st := txnGoesBelowMinBal.Sign(keys[0])
	err = eval.Transaction(st, transactions.ApplyData{})
	require.Error(t, err)
	// TODO dissect the error message
