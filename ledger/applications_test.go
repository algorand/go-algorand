...
return`
	...
	a.NoError(err)
	a.NotEmpty(pad)
	prd, err := l.accts.accountsq.lookupResources(userOptin, basics.CreatableIndex(appIdx))
	a.NoError(err)
...
return`
	...
	a.Zero(pad.rowid)
	prd, err := l.accts.accountsq.lookupResources(userLocal, basics.CreatableIndex(appIdx))
	a.NoError(err)
	a.Zero(prd.addrid)
...
return`
	...
	a.Zero(pad.rowid)
	prd, err := l.accts.accountsq.lookupResources(creator, basics.CreatableIndex(appIdx))
	a.NoError(err)
	a.Zero(prd.addrid)
	emptyResourceData := makeResourcesData(0)
	a.Equal(emptyResourceData, prd.data)
