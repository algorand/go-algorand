...
func (x *roundCowBase) lookupCreatableData(addr basics.Address, locators []basics.CreatableLocator) (data ledgercore.PersistedAccountData, err error) {
...
func (cs *roundCowState) GetEx(addr basics.Address, cidx basics.CreatableIndex, ctypes []basics.CreatableType) (basics.AccountData, error) {
	if !cs.proto.EnableUnlimitedAssets {
		return cs.Get(addr, false)
	}

	locators := make([]basics.CreatableLocator, len(ctypes))
	for i, ctype := range ctypes {
		locators[i] = basics.CreatableLocator{Type: ctype, Index: cidx}
	}
	pad, err := cs.lookupCreatableData(addr, locators)
	if err != nil {
		return basics.AccountData{}, err
	}
	return pad.AccountData, nil
...
type ledgerForCowBase interface {
	...
	lookupCreatableDataWithoutRewards(basics.Round, basics.Address, []basics.CreatableLocator) (ledgercore.PersistedAccountData, error)
