...
func TestPayTxn(t *testing.T) {
	...
	for _, signed := range []bool{true, false} {
		t.Run(fmt.Sprintf("signed=%t", signed), func(t *testing.T) {
			...
			require.Equal(t, result.WouldSucceed, signed)
			require.Len(t, result.TxnGroups, 1)
			require.Len(t, result.TxnGroups[0].Txns, 1)
			require.Equal(t, result.TxnGroups[0].Txns[0].MissingSignature, !signed)
...
func TestOverspendPayTxn(t *testing.T) {
	...
	for _, signed := range []bool{true, false} {
		t.Run(fmt.Sprintf("signed=%t", signed), func(t *testing.T) {
			...
			require.Len(t, result.TxnGroups[0].Txns, 1)
			require.Equal(t, result.TxnGroups[0].Txns[0].MissingSignature, !signed)
			require.Contains(t, result.TxnGroups[0].FailureMessage, fmt.Sprintf("tried to spend {%d}", amount))
		})
	}
}
...
func TestAuthAddrTxn(t *testing.T) {
	...
	for _, signed := range []bool{true, false} {
		t.Run(fmt.Sprintf("signed=%t", signed), func(t *testing.T) {
			...
			require.Len(t, result.TxnGroups[0].Txns, 1)
			require.Equal(t, result.TxnGroups[0].Txns[0].MissingSignature, !signed)
			require.Contains(t, result.TxnGroups[0].FailureMessage, fmt.Sprintf("should have been authorized by %s but was actually authorized by %s", sender.Addr, authority.Addr))
		})
	}
}
...
func TestStateProofTxn(t *testing.T) {
	...
	l, _, makeTxnHeader := simulationtesting.PrepareSimulatorTest(t)
	defer l.Close()
	s := simulation.MakeSimulator(l)
