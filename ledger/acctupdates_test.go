...
func TestBoxKVNoCache(t *testing.T) {
	partitiontest.PartitionTest(t)
	t.Parallel()
	...
	ledgertesting.TestConsensusRange(t, boxVersion, 0, func(t *testing.T, ver int, cv protocol.ConsensusVersion, cfg config.Local) {
		...
		protoParams := config.Consensus[cv]

		ml := makeMockLedgerForTracker(t, true, initialBlocksCount, cv,
			[]map[basics.Address]basics.AccountData{accts},
		)
		defer ml.Close()
		...
		addKV2AU := func(appID basics.AppIndex, boxKey, boxValue []byte) {
			boxChange := ledgercore.KvValueDelta{Data: boxValue}
			auNewBlock(t, currentRound, au, accts, opts, map[string]ledgercore.KvValueDelta{
				apps.MakeBoxKey(uint64(appID), string(boxKey)): boxChange,
			})
			auCommitSync(t, currentRound, au, ml)
			...
			rnd := au.latest()
			require.Equal(t, currentRound, rnd)
			if uint64(currentRound) > cfg.MaxAcctLookback {
				require.Equal(t, currentRound-basics.Round(cfg.MaxAcctLookback), au.cachedDBRound)
			} else {
				require.Equal(t, basics.Round(0), au.cachedDBRound)
			}
			currentRound++
		...
		readKey := func(appID basics.AppIndex, boxKey []byte) ([]byte, error) {
			res, err := au.LookupKv(currentRound-1, apps.MakeBoxKey(uint64(appID), string(boxKey)))
			return res, err
		}
		...
		testcases := []struct {
			...
			{appID: 10, boxKey: []byte("alice"), boxValue: []byte("value-alice")},
			{appID: 20, boxKey: []byte("bob--"), boxValue: []byte("value-bob--")},
			{appID: 30, boxKey: []byte("carol"), boxValue: []byte("value-carol")},
			{appID: 40, boxKey: []byte("dave-"), boxValue: []byte("value-dave-")},
			{appID: 50, boxKey: []byte("frank"), boxValue: []byte("value-frank")},
		}

		addKV2AU(testcases[0].appID, testcases[0].boxKey, testcases[0].boxValue)
		addKV2AU(testcases[1].appID, testcases[1].boxKey, testcases[1].boxValue)
		addKV2AU(testcases[2].appID, testcases[2].boxKey, testcases[2].boxValue)
		addKV2AU(testcases[3].appID, testcases[3].boxKey, testcases[3].boxValue)
		val, err := readKey(testcases[0].appID, testcases[0].boxKey)
		require.NoError(t, err)
