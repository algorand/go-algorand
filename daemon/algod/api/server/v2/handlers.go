...
func (v2 *Handlers) RawTransaction(ctx echo.Context) error {
	...
	if err = validatePQSignaturesForAPI(proto, txgroup, false, false); err != nil {
		return badRequest(ctx, err, err.Error(), v2.Log)
	}
...
func (v2 *Handlers) SimulateTransaction(ctx echo.Context, params model.SimulateTransactionParams) error {
	...
	for groupIdx, txgroup := range simulateRequest.TxnGroups {
		if err = validatePQSignaturesForAPI(proto, txgroup.Txns, simulateRequest.AllowEmptySignatures, simulateRequest.FixSigners); err != nil {
