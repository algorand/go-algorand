...
func NewRouter(logger logging.Logger, handler v2.HandlerInterface, apiToken string, adminAPIToken string, numConnectionsLimit uint64) *echo.Echo {
	...
	if handler.GetNode().Config().EnableProfiler {
	...
	handler.Register(e, apiAuthenticator, adminAuthenticator)
