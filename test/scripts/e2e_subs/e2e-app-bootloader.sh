...
APPID=$(${gcmd} app create --creator ${ACCOUNT} --approval-prog ${TEMPDIR}/bootloader.teal --global-byteslices 1 --global-ints 0 --no-local-state --clear-prog "${TEMPDIR}/int1.teal" | grep Created | awk '{ print $6 }')
