...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to optIn(string)string should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}" ]]; then
    date "+${scriptname} FAIL the method call to empty()void should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to add(uint64,uint64)uint64 should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to payment(pay,uint64)bool should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to referenceTest(account,application,account,asset,account,asset,asset,application,application)uint8[9] should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to closeOut()string should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to update()void should not fail %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the method call to delete()void should not fail %Y%m%d_%H%M%S"
    false
fi
...
EXPECTED="method referenceTest(account,application,account,asset,account,asset,asset,application,application)uint8[9] succeeded with output: [2,0,2,0,2,1,0,1,0]"
if [[ $RES != *"${EXPECTED}"* ]]; then
    date "+${scriptname} FAIL the creation method call to referenceTest(account,application,account,asset,account,asset,asset,application,application)uint8[9] should not fail %Y%m%d_%H%M%S"
    false
fi
