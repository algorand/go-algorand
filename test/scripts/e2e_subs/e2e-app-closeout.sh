...
APPID=$(${gcmd} app create --creator ${ACCOUNT} --approval-prog <(echo 'int 1') --global-byteslices 1 --global-ints 1 --local-byteslices 1 --local-ints 1 --clear-prog <(echo 'int 1') | grep Created | awk '{ print $6 }')
