...
APP_ID=$(${gcmd} app create --creator ${ACCOUNT} --app-arg "int:$ASSET_ID" --app-arg "int:1337" --app-arg "int:0" --app-arg "int:0" --app-arg "int:1337" --app-arg "str:bogo" --app-arg "int:$ROUND" --approval-prog ${DIR}/tealprogs/assetround.teal --global-byteslices 0 --global-ints 0 --local-byteslices 0 --local-ints 0 --clear-prog <(echo "int 1") | grep Created | awk '{ print $6 }')
...
ROUND=$(goal node status | grep 'Last committed' | awk '{ print $4 }')
${gcmd} app call --app-id $APP_ID --from $ACCOUNTB --app-arg "int:$ASSET_ID" --app-arg "int:17" --app-arg "int:0" --app-arg "int:1" --app-arg "str:" --app-arg "str:" --app-arg "int:$ROUND"
...
# Check bit flipped
ROUND=$(goal node status | grep 'Last committed' | awk '{ print $4 }')
${gcmd} app call --app-id $APP_ID --from $ACCOUNTB --app-arg "int:$ASSET_ID" --app-arg "int:17" --app-arg "int:1" --app-arg "int:1" --app-arg "str:" --app-arg "str:" --app-arg "int:$ROUND"
