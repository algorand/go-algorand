...
ACCOUNTA=$(${gcmd} account new|tee ${OUTPUT_DEVICE}|awk '{ print $6 }')
...
while [ ${ROUND} != ${PBOUND} ]; do
    goal node wait
    ROUND=$(goal node status |tee ${OUTPUT_DEVICE}| grep 'Last committed block:'|awk '{ print $4 }')
done
...
REGOK=$(${gcmd} account list |tee ${OUTPUT_DEVICE}| grep ${ACCOUNTA} | awk '{ print $1 }' | grep -c online)
if [[ $REGOK != 1 ]]; then
...
ROUND=$(goal node status | grep 'Last committed block:'|tee ${OUTPUT_DEVICE}|awk '{ print $4 }')
