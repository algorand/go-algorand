...
EXPERROR="static cost budget of 700 exceeded"
if [[ $RES != *"${EXPERROR}"* ]]; then
...
EXPERROR="dynamic cost budget exceeded, executing intc_0: local program cost was 700"
if [[ $RES != *"${EXPERROR}"* ]]; then
...
if [[ $RES != *"${EXP}"* ]]; then
    date "+${scriptname} FAIL the application creation should pass %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${PROGHASH}"* ]]; then
    date "+${scriptname} FAIL the application approval program hash is incorrect %Y%m%d_%H%M%S"
    false
fi
if [[ $RES != *"${EXTRAPAGES}"* ]]; then
    date "+${scriptname} FAIL the application extra pages value is incorrect %Y%m%d_%H%M%S"
    false
fi
if [[ $RES == *version* ]]; then
    date "+${scriptname} FAIL a new application should not show Program version %Y%m%d_%H%M%S"
    false
fi
...
if [[ $RES != *"${EXP}"* ]]; then
    date "+${scriptname} FAIL the application update should succeed %Y%m%d_%H%M%S"
    false
fi

RES=$(${gcmd} app info --app-id ${APPID} || true)
if [[ $RES == *"${PROGHASH}"* ]]; then
    date "+${scriptname} FAIL the application approval program should have been updated %Y%m%d_%H%M%S"
    false
fi
if [[ $RES != *"${EXTRAPAGES}"* ]]; then
    date "+${scriptname} FAIL the application extra pages value is incorrect after update %Y%m%d_%H%M%S"
    false
fi
