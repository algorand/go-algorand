...
APPID=$(${gcmd} app create --creator "${ACCOUNT}" --approval-prog=${TEAL}/check_creatable_id.teal --global-byteslices 0 --global-ints 0 --local-byteslices 0 --local-ints 0  --clear-prog=${TEAL}/approve-all.teal | grep Created | awk '{ print $6 }')
...
${gcmd} app call --app-id="$APPID" --from="$ACCOUNT" --app-arg=int:0 --out "$TEMPDIR/unsigned_asset_check_app_call.txn"
...
${gcmd} app call --app-id="$APPID" --from="$ACCOUNT" --app-arg=int:2 --out "$TEMPDIR/unsigned_app_check_app_call.txn"
