...
ACCOUNT_TLHC=$(${gcmd} clerk compile -n ${TEMPDIR}/tlhc.teal|tee ${OUTPUT_DEVICE}|awk '{ print $2 }')
...
CROUND=$(goal node status | grep 'Last committed block:'|tee ${OUTPUT_DEVICE}|awk '{ print $4 }')
...
ACCOUNT_TRUE=$(${gcmd} clerk compile -n ${TEMPDIR}/true.teal|tee ${OUTPUT_DEVICE}|awk '{ print $2 }')
...
ACCOUNTM=$(${gcmd} account multisig new -T 2 ${ACCOUNT} ${ACCOUNTB} ${ACCOUNTC}|tee ${OUTPUT_DEVICE}|awk '{ print $6 }')
