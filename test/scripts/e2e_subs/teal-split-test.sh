...
ACCOUNTC=$(${gcmd} account new|tee ${OUTPUT_DEVICE}|awk '{ print $6 }')
...
ACCOUNT_SPLIT=$(${gcmd} clerk compile ${TEMPDIR}/split.teal -o ${TEMPDIR}/split.tealc|tee ${OUTPUT_DEVICE}|awk '{ print $2 }')
...
BALANCEC=$(${gcmd} account balance -a ${ACCOUNTC}|tee ${OUTPUT_DEVICE}|awk '{ print $1 }')
