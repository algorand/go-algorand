...
type algohExpectFixture struct {
	testDir     string
	testDataDir string
	testDirTmp  bool
	t           *testing.T
...
func (f *algohExpectFixture) initialize(t *testing.T) (err error) {
	f.t = t
...
func (f *algohExpectFixture) getTestDir(testName string) (workingDir, algoDir string, err error) {
	...
	algoDir = filepath.Join(workingDir, "algoh")
...
func (f *algohExpectFixture) removeTestDir(workingDir string) (err error) {
...
// TesthWithExpect Process all expect script files with suffix Test.exp within the test/e2e-go/cli/algoh/expect directory
func TestAlgohWithExpect(t *testing.T) {
	var f algohExpectFixture
