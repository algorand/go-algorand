...
func main() {
	...
	if !isTest {
		...
		f.WriteString(fmt.Sprintf("%s: telemetryConfig.SendToLog: %v telemetryConfig.Enable %v  cfg.TelemetryToLog %v\n", time.Now().String(), telemetryConfig.SendToLog,  telemetryConfig.Enable, cfg.TelemetryToLog))
		f.Close()
