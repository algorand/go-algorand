...
func TestDebugFromPrograms(t *testing.T) {
	...
	err := l.Setup(&dp)
	a.ErrorContains(err, `invalid group index 1 for a single transaction`)
	...
	err = l.Setup(&dp)
	a.ErrorContains(err, `invalid group index 3 for a txn in a transaction group of 2`)
	...
	err = l.Setup(&dp)
	a.ErrorContains(err, `unknown run mode`)
...
func TestDebugFromTxn(t *testing.T) {
	...
	err = l.Setup(&dp)
	a.ErrorContains(err, `no programs found in transactions`)
