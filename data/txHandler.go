...
type txBacklogMsg struct {
	...
	peers                 map[network.Peer]struct{} // peers that ever sent this message
}
...
func (handler *TxHandler) postProcessCheckedTxn(wi *txBacklogMsg) {
	...
	handler.net.RelayArray(
		handler.ctx,
		[]protocol.Tag{protocol.TxnTag},
		[][]byte{reencode(verifiedTxGroup)},
		false, wi.peers)
}
...
func (handler *TxHandler) processIncomingTxn(rawmsg network.IncomingMessage) network.OutgoingMessage {
	...
	var isDup bool
	var peers map[network.Peer]struct{}
	if handler.msgCache != nil {
		// check for duplicate messages
		// this helps against relaying duplicates
		if msgKey, peers, isDup = handler.msgCache.CheckAndPut(rawmsg.Data, rawmsg.Sender); isDup {
