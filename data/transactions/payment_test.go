...
func TestCheckSpender(t *testing.T) {
	mockBalV0 := mockBalances{protocol.ConsensusCurrentVersion}
	mockBalV10 := mockBalances{protocol.ConsensusV10}
	...
	require.Error(t, tx.checkSpender(tx.Header, spec, mockBalV0.ConsensusParams()))
	require.Error(t, tx.checkSpender(tx.Header, spec, mockBalV10.ConsensusParams()))

	tx.Sender = src
	require.NoError(t, tx.checkSpender(tx.Header, spec, mockBalV10.ConsensusParams()))
}
