...
func TestGetNumberOfBatchableSigsInGroup(t *testing.T) {
	...
	// txn with 0 sigs
	txnGroups[mod][0].Sig = crypto.Signature{}
	batchSigs, err := UnverifiedElement{TxnGroup: txnGroups[mod]}.GetNumberOfBatchableSigsInGroup()
	...
	batchSigs, err = UnverifiedElement{TxnGroup: mSigTxn}.GetNumberOfBatchableSigsInGroup()
	require.NoError(t, err)
...
func TestStreamVerifierPoolShutdown(t *testing.T) { //nolint:paralleltest // Not parallel because it depends on the default logger
	...
	for err := range errChan {
		require.ErrorIs(t, err, streamv.ErrShuttingDownError)
	}
...
func TestStreamVerifierCtxCancel(t *testing.T) {
	...
	wg.Wait()
	require.ErrorIs(t, result.Err, streamv.ErrShuttingDownError)
}
...
func TestStreamVerifierCtxCancelPoolQueue(t *testing.T) { //nolint:paralleltest // Not parallel because it depends on the default logger
	...
	go func() {
		...
		for {
			...
			if result.Err != streamv.ErrShuttingDownError {
				continue
			}
			break
		}
	...
	time.Sleep(6 * streamv.WaitForNextTxnDuration)
	go func() {
