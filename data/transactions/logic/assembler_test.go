...
`
	...
	require.Equal(t, SourceLocation{Line: 2}, location)
	// err 3
	location, ok = ops.OffsetToSource[prefixLen+5]
	require.True(t, ok)
...
`, AssemblerMaxVersion)
	...
	`,
		...
		strings.Join(labels, ":\n")+":\n")
	ops = testProg(t, source, AssemblerMaxVersion)
	requireProgramLen(t, source, AssemblerMaxVersion, ops.Program, 1024) // prefix + pushints (2*256) + opcode (1) + length (1) + labels (2*255)
