...
`
	...
	require.Equal(t, prefixLen+5, len(ops.Program))
...
`, AssemblerMaxVersion)
	...
	`,
		...
		strings.Join(labels, ":\n")+":\n")
	ops = testProg(t, source, AssemblerMaxVersion)
	require.Len(t, ops.Program, programPrefixLen(t, ops.Program)+1024) // prefix + pushints (2*256) + opcode (1) + length (1) + labels (2*255)
