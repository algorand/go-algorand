...
var opDocExtraList = []stringString{
	...
	{"stxn", "for notes on transaction fields available, see `txn`. If top of stack is _i_, `stxn field` is equivalent to `gtxn _i_ field`. stxn exists so that _i_ can be calculated, often based on the index of the current transaction."},
	{"btoi", "`btoi` panics if the input is longer than 8 bytes."},
	{"concat", "`concat` panics if the result would be greater than 4096 bytes."},
	{"getbit", "bit indexing begins with low-order bits in integers. Bit 4 is the 1 in int 0x0010. Indexing begins in the first bytes of a byte-string (as seen in getbyte and substring). Bits 0 through 11 are 1 in byte 0xfff000000000"},
	{"setbit", "see explanation of bit ordering in getbit"},
