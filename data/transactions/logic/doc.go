...
var opDocByName = map[string]string{
	...
	"holding_get":         "X is field F from account A's holding of asset B. Y is 1 if A is opted into B, else 0",
	"asset_get":           "X is field F from asset A. Y is 1 if A exists, else 0",
	"app_get":             "X is field F from app A. Y is 1 if A exists, else 0",
	"acct_get":            "X is field F from account A. Y is 1 if A owns positive algos, else 0",
...
var opcodeImmediateNotes = map[string]string{
	...
	"holding_get": "{uint8 asset holding field index}",
	"asset_get":   "{uint8 asset params field index}",
	"app_get":     "{uint8 app params field index}",
	"acct_get":    "{uint8 account params field index}",
...
var opDocExtras = map[string]string{
	...
	"opted_in":            "params: An _available_ account address and an _available_ application id. Return: 1 if opted in and 0 otherwise.",
	"local_state_get":     "params: An _available_ account address, state key. Return: value. The value is zero (of type uint64) if the key does not exist.",
	"local_state_get_ex":  "params: An _available_ account address, an _available_ application id, state key. Return: did_exist flag (top of the stack, 1 if the application and key existed and 0 otherwise), value. The value is zero (of type uint64) if the key does not exist.",
	"global_state_get_ex": "params: An _available_ application id, state key. Return: did_exist flag (top of the stack, 1 if the application and key existed and 0 otherwise), value. The value is zero (of type uint64) if the key does not exist.",
	"global_state_get":    "params: state key. Return: value. The value is zero (of type uint64) if the key does not exist.",
	"local_state_put":     "params: An _available_ account address, state key, value.",
	"local_state_del":     "params: An _available_ account address, state key.\n\nDeleting a key which is already absent has no effect on the application local state. (In particular, it does _not_ cause the program to fail.)",
	"global_state_del":    "params: state key.\n\nDeleting a key which is already absent has no effect on the application global state. (In particular, it does _not_ cause the program to fail.)",
	"holding_get":         "params: An _available_ address, an _available_ asset id. Return: did_exist flag (1 if the asset existed and 0 otherwise), value.",
	"asset_get":           "params: An _available_ asset id. Return: did_exist flag (1 if the asset existed and 0 otherwise), value.",
	"app_get":             "params: An _available_ app id. Return: did_exist flag (1 if the application existed and 0 otherwise), value.",
...
var OpGroups = map[string][]string{
	...
	"State Access":            {"balance", "min_balance", "app_opted_in", "opted_in", "app_local_get", "local_state_get", "app_local_get_ex", "local_state_get_ex", "app_global_get", "global_state_get", "app_global_get_ex", "global_state_get_ex", "app_local_put", "local_state_put", "app_global_put", "global_state_put", "app_local_del", "local_state_del", "app_global_del", "global_state_del", "asset_holding_get", "holding_get", "asset_params_get", "asset_get", "app_params_get", "app_get", "acct_params_get", "acct_get", "log", "block"},
