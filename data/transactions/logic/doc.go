...
var opDocByName = map[string]string{
	...
	"decode_uvarint": "pop a byte-array A and integer B. Extract a range of bytes from A starting at B up to but not including B+8, convert bytes as big endian and push the uint64 result and number of bytes read.",
	"encode_uvarint": "pop a uint64 A. Encode A as big endiand uvarint bytes and push the byte result.",
...
var OpGroups = map[string][]string{
	...
	"Uvarint encoding":      {"decode_uvarint", "encode_uvarint"},
