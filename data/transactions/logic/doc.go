...
var opDocExtras = map[string]string{
	...
	"bn256_add":           "A, B is curve point in G1 group. Each point is consists of (X, Y). X and Y are both 256 bit integer, big-endian encoded. The encoded point is 64 bytes from concatenation of 32 byte X and 32 byte Y.",
	"bn256_scalar_mul":    "A is curve point in G1 Group and encoding described in `bn256_add`. Scalar K is a big-endian encoded big integer that has no padding zeros.",
	"bn256_pairing":       "G1s are encoded by concat each encoded G1, which described in `bn256_add`. G2s are encoded by concat encoded G2. Each G2 is in form (XA0+i*XA1, YA0+i*YA1) and encoded by big-endian field element XA0, XA1, YA0 and YA1 in sequence.",
