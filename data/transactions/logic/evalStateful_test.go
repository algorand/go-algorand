...
func TestEnumFieldErrors(t *testing.T) { // nolint:paralleltest // manipulates globalFieldSpecs
	...
	changed := origSpec
	changed.ftype.AVMType = avmBytes
	...
	badMinTxnFs.ftype.AVMType = avmBytes
...
`
	...
	badBalanceFs.ftype.AVMType = avmBytes
...
`
	...
	badTotalFs.ftype.AVMType = avmBytes
		...
		for name, spec := range byName {
			...
			t.Run(fmt.Sprintf("mode=%s,opcode=%s", m, name), func(t *testing.T) {
				...
				for i := 0; i < len(spec.Return.Types); i++ {
					...
					require.True(
						t, avmType.stackType().AssignableTo(retType),
