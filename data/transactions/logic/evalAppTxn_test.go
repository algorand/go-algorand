...
// TestInnerTxnStrangeFields logs a few fields that are handled oddly (valid
// rounds are copied, GenesisHash is "virtually copied") so we can check if they
// are correct.
func TestInnerTxnStrangeFields(t *testing.T) {
	partitiontest.PartitionTest(t)
	t.Parallel()
	ep, tx, ledger := MakeSampleEnv()
...
itxn Logs 2; txn GenesisHash; ==; assert
int 1
`, ep)
