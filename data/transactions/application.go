...
func (ac *ApplicationCallTxnFields) applyEvalDelta(evalDelta basics.EvalDelta, params basics.AppParams, creator, sender basics.Address, balances Balances, appIdx basics.AppIndex) error {
	/*
...
func (ac *ApplicationCallTxnFields) applyClearState(
	...
	if exists {
		...
		if err == nil && pass {
			...
			err = ac.applyEvalDelta(evalDelta, params, creator, sender, balances, appIdx)
...
func (ac *ApplicationCallTxnFields) apply(header Header, balances Balances, spec SpecialAddresses, ad *ApplyData, txnCounter uint64, steva StateEvaluator) (err error) {
	...
	err = ac.applyEvalDelta(evalDelta, params, creator, header.Sender, balances, appIdx)
	if err != nil {
		return err
	}
