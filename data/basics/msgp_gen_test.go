...
func TestMarshalUnmarshalPQScheme(t *testing.T) {
	partitiontest.PartitionTest(t)
	v := PQScheme{}
	bts := v.MarshalMsg(nil)
	left, err := v.UnmarshalMsg(bts)
...
func TestRandomizedEncodingPQScheme(t *testing.T) {
	protocol.RunEncodingTest(t, &PQScheme{})
}

func BenchmarkMarshalMsgPQScheme(b *testing.B) {
	v := PQScheme{}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkAppendMsgPQScheme(b *testing.B) {
	v := PQScheme{}
	bts := make([]byte, 0, v.Msgsize())
	bts = v.MarshalMsg(bts[0:0])
	b.SetBytes(int64(len(bts)))
...
func BenchmarkUnmarshalPQScheme(b *testing.B) {
	v := PQScheme{}
	bts := v.MarshalMsg(nil)
	b.ReportAllocs()
	b.SetBytes(int64(len(bts)))
