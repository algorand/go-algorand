...
func (m mockSender) IPAddr() []byte      { return m.addr[:] }
...
func makeTestTxHandler(dl *Ledger, cfg config.Local) (*TxHandler, *pools.TransactionPool, error) {
	...
	hander, err := MakeTxHandler(opts)
	return hander, tp, err
}
...
func (m *mockTxPool) Remember(txg []transactions.SignedTxn) error {
	time.Sleep(time.Duration(m.appCost) * 10 * time.Microsecond)
...
func TestTxHandlerAppRateLimiter_PenalizeEvalError(t *testing.T) {
	...
	t.Skip("This is 2s test with lots of knobs, better when running manually")
