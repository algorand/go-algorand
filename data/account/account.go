...
func ImportRoot(store db.Accessor, seed [32]byte) (acc Root, err error) {
	...
	err = store.Atomic("accountImport", func(tx *sql.Tx) error {
...
func RestoreRoot(store db.Accessor) (acc Root, err error) {
	...
	err = store.Atomic("accountRestore", func(tx *sql.Tx) error {
		var nrows int
...
func RestoreParticipation(store db.Accessor) (acc Participation, err error) {
	...
	err = store.Atomic("accountRestoreParticipation", func(tx *sql.Tx) error {
		var nrows int
