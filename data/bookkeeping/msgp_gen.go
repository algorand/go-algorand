...
func (z *Block) MarshalMsg(b []byte) (o []byte) {
	...
	if (*z).BlockHeader.BaseFee.MsgIsZero() {
		zb0005Len--
		zb0005Mask |= 0x20
	}
	...
	if zb0005Len != 0 {
		if (zb0005Mask & 0x20) == 0 { // if not empty
			...
			o = (*z).BlockHeader.BaseFee.MarshalMsg(o)
		}
		...
		if (zb0005Mask & 0x2000) == 0 { // if not empty
			...
			o = msgp.AppendUint64(o, (*z).BlockHeader.Load)
		}
...
func (z *Block) UnmarshalMsgWithState(bts []byte, st msgp.UnmarshalState) (o []byte, err error) {
	...
	if _, ok := err.(msgp.TypeError); ok {
		...
		if zb0005 > 0 {
			zb0005--
			(*z).BlockHeader.Load, bts, err = msgp.ReadUint64Bytes(bts)
			if err != nil {
		...
		if zb0005 > 0 {
			zb0005--
			bts, err = (*z).BlockHeader.BaseFee.UnmarshalMsgWithState(bts, st)
			if err != nil {
				err = msgp.WrapError(err, "struct-from-array", "BaseFee")
				return
			}
		}
		if zb0005 > 0 {
...
func BlockMaxSize() (s int) {
	...
	s += 3 + msgp.Uint64Size + 3 + basics.MicroAlgosMaxSize() + 5
...
func (z *BlockHeader) UnmarshalMsgWithState(bts []byte, st msgp.UnmarshalState) (o []byte, err error) {
	...
	if _, ok := err.(msgp.TypeError); ok {
		...
		if zb0005 > 0 {
			zb0005--
			(*z).Load, bts, err = msgp.ReadUint64Bytes(bts)
			if err != nil {
				err = msgp.WrapError(err, "struct-from-array", "Load")
				return
			}
		...
		if zb0005 > 0 {
			zb0005--
			bts, err = (*z).BaseFee.UnmarshalMsgWithState(bts, st)
			if err != nil {
				err = msgp.WrapError(err, "struct-from-array", "BaseFee")
				return
			}
...
func BlockHeaderMaxSize() (s int) {
	...
	s += msgp.ArrayHeaderSize + ((bounds.MaxMarkAbsent) * (basics.AddressMaxSize()))
	s += 3 + msgp.Uint64Size + 3 + basics.MicroAlgosMaxSize()
	return
}
