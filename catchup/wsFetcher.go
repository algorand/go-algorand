...
func (w *wsFetcherClient) RequestBlock(ctx context.Context, round basics.Round) (rpcs.WsGetBlockOut, error) {
	...
	if err != nil {
		return rpcs.WsGetBlockOut{}, fmt.Errorf("WsFetcherService(%s).RequestBlock(%d): Request failed, %v", w.target.GetAddress(), round, err)
	}

	if errMsg, found := resp.Topics.GetValue(network.ErrorKey); found {
		return rpcs.WsGetBlockOut{}, fmt.Errorf("WsFetcherService(%s).RequestBlock(%d): Request failed, %s", w.target.GetAddress(), round, string(errMsg))
	}

	blk, found := resp.Topics.GetValue(rpcs.BlockDataKey)
	if !found {
		return rpcs.WsGetBlockOut{}, fmt.Errorf("WsFetcherService(%s): request failed: block data not found", w.target.GetAddress())
	}
	cert, found := resp.Topics.GetValue(rpcs.CertDataKey)
	if !found {
		return rpcs.WsGetBlockOut{}, fmt.Errorf("WsFetcherService(%s): request failed: cert data not found", w.target.GetAddress())
	}
