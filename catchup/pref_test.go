...
func BenchmarkServiceFetchBlocks(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		local, err := data.LoadLedger(logging.TestingLog(b), prefix, prefix, inMem, protocol.ConsensusCurrentVersion, genesisBalances, "", crypto.Digest{}, nil, cfg)
...
func benchenv(t testing.TB, numAccounts, numBlocks int) (ledger, emptyLedger *data.Ledger, release func(), genesisBalances bookkeeping.GenesisBalances) {
	...
	cfg.Archival = true
	prefix := t.Name() + "empty"
	emptyLedger, err = data.LoadLedger(logging.TestingLog(t), prefix, prefix, inMem, protocol.ConsensusCurrentVersion, genesisBalances, "", crypto.Digest{}, nil, cfg)
