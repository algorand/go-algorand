...
func TestValidRounds(t *testing.T) {
	...
	a.ErrorContains(err, `cannot construct transaction: txn validity period 1001 is greater than protocol max txn lifetime 1000`)
	...
	validRounds = 2
	_, _, err = computeValidityRounds(firstValid, lastValid, validRounds, lastRound, maxTxnLife)
	a.ErrorContains(err, `cannot construct transaction: ambiguous input: lastValid = 1, validRounds = 2`)
	...
	validRounds = 0
	_, _, err = computeValidityRounds(firstValid, lastValid, validRounds, lastRound, maxTxnLife)
	a.ErrorContains(err, `cannot construct transaction: txn would first be valid on round 2 which is after last valid round 1`)
	...
	validRounds = 0
	_, _, err = computeValidityRounds(firstValid, lastValid, validRounds, lastRound, maxTxnLife)
	a.ErrorContains(err, `cannot construct transaction: txn validity period ( 1 to 1002 ) is greater than protocol max txn lifetime 1000`)
