...
If all goes well, the run will generate a directory `tmp/OUTPUT_RUN_RUNNER_TEST` and in that directory you can see the statistics
...
0. We assume that the `conduit` binary is available in the same directory as `run_runner.py`
1. You'll need to be in a python virtual env that has the [requirements](./requirements.txt) installed.
2. Let's run a similar scenario as above, but with:
...
4. What is the docker container?
...
5. Let's enter a `psql` session insdie the docker container and query the database:
...
6. Let's shutdown the container:
