...
func init() {
	...
	RunnerCmd.Flags().BoolVarP(&runnerArgs.RunValidation, "validate", "", false, "If set, the runner will run after test-duration has elapsed to verify data is correct. An extra line in each report indicates validator success or failure.")
	RunnerCmd.Flags().BoolVarP(&runnerArgs.KeepDataDir, "keep-data-dir", "k", false, "If set, the runner will not delete the data directory after tests complete.")
	RunnerCmd.Flags().StringVarP(&runnerArgs.GenesisFile, "genesis-file", "f", "", "The file path to the genesis associated with the db snapshot.")
	RunnerCmd.Flags().BoolVarP(&runnerArgs.ResetDB, "reset-db", "", false, "If set, the database will be deleted before running tests.")
	RunnerCmd.Flags().Uint64VarP(&runnerArgs.Times, "times", "t", 1, "The number of times to run the scenario(s).")
