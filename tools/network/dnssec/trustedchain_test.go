...
func TestEnsureTrustChain(t *testing.T) {
	...
	err = tch.ensure(context.Background(), "algo.test.", []uint16{algoTestZSK.KeyTag()})
	a.ErrorContains(err, `no KSK to verify DNSKEY RRSet of test.`)
	...
	a.ErrorContains(err, `failed to verify test. KSK against digest in parent DS`)
...
func TestEnsureTrustChainError(t *testing.T) {
	...
	a.ErrorContains(err, `is not FQDN`)
	// test non-existing ZSK
	err = newCh.ensure(context.Background(), ".", []uint16{})
	a.ErrorContains(err, `ZSK [] not found in zone .`)
	...
	tch := makeTrustChain(r)
	err = tch.ensure(context.Background(), "test.", []uint16{testZSK.KeyTag()})
	a.ErrorContains(err, `cache outdated for already updated zone .`)
...
func TestAuthenticate(t *testing.T) {
	...
	a.ErrorContains(err, `signer name mismatch: . vs test`)
	...
	err = tch.Authenticate(context.Background(), rrset, rrsig)
	a.ErrorContains(err, `not found in zone`)

	err = tch.Authenticate(context.Background(), rrset, nil)
	a.ErrorContains(err, `empty RRSIG`)
