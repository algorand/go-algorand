...
func encode(t *clockManager, rr rootRouter, p serializableActor, a []action) []byte {
	var s diskState
	s.Router = protocol.EncodeReflect(rr)
	s.Player = encodePlayer(p)
	s.Clock = t.Encode()
	for _, act := range a {
...
func decode(raw []byte, t0 *clockManager) (t *clockManager, rr rootRouter, p serializableActor, a []action, err error) {
	var t2 *clockManager
	var rr2 rootRouter
	var p2 serializableActor
	a2 := []action{}
	var s diskState
...
type playerDiskState struct {
	PlayerType uint
	Buf        []byte
}

func decodePlayer(buf []byte) (serializableActor, error) {
	var s playerDiskState
	err := protocol.DecodeReflect(buf, &s)
	if err != nil {
		return nil, err
	}
	switch s.PlayerType {
	case 0: // XXX use enum
		var p player
		return p.decode(s.Buf)
	case 1:
		var p pipelinePlayer
		return p.decode(s.Buf)
	default:
		return nil, fmt.Errorf("couldn't decode player state")
	}
}

func encodePlayer(p serializableActor) []byte {
	buf := p.encode()
	switch p.(type) {
	case *player:
		return protocol.EncodeReflect(&playerDiskState{PlayerType: 0, Buf: buf})
	case *pipelinePlayer:
		return protocol.EncodeReflect(&playerDiskState{PlayerType: 1, Buf: buf})
	default:
...
// Enqueue takes a clockManager, the RPS of all pipelined players, the RPS of the player that requested persisting,
// the raw encoding of the (clockManager, router, pipelinePlayer, persistAction) a done channel, and queues it up for writing
// to disk by asyncPersistenceLoop.loop.
func (p *asyncPersistenceLoop) Enqueue(clockManager *clockManager, pipelineRPS []RPS, playerRPS RPS, raw []byte, done chan error) (events <-chan externalEvent) {
...
func (p *asyncPersistenceLoop) loop(ctx context.Context) {
	...
	for {
		...
		// XXX call ledger.Wait on all players in pipelinePlayer iteratively
		// XXX API that distinguishes on Wait from WaitSpeculative
		// make sure that the ledger finished writing the previous round to disk.
		for _, rps := range s.allRPS {
			select {
			case <-ctx.Done():
				return
			case <-p.ledger.Wait(rps.Round.Number.SubSaturate(1), rps.Round.Branch):
			}
		}

		// modify persistentRequest to take pipelinePlayer

		// store the state.
		// XXX playerRPS that is passed in is only used for logging
		err := persist(p.log, p.crashDb, s.playerRPS.Round, s.playerRPS.Period, s.playerRPS.Step, s.raw)

		// XXX playerRPS provided to checkpointEvent is handled by corresponding player and used for log information in
		// checkpointAction generated by player.
		s.events <- checkpointEvent{
			Round:  s.playerRPS.Round,
			Period: s.playerRPS.Period,
			Step:   s.playerRPS.Step,
			Err:    makeSerErr(err),
			done:   s.done,
		}
		close(s.events)
