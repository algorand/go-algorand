...
func makeDemux(params demuxParams) (d *demux) {
	d = new(demux)
	d.crypto = makeCryptoVerifier(params.validator, params.voteVerifier, params.log)
	d.log = params.log
...
func (d *demux) verifyVote(ctx context.Context, m message, taskIndex int, r round, p period, b bookkeeping.BlockHash) {
	...
	params, err := l.ConsensusParams(ParamsRound(r.Number))
	if err != nil && !params.AgreementMessagesContainBranch {
...
func (d *demux) next(s *Service, extSignals pipelineExternalDemuxSignals) (e externalEvent, ok bool) {
	...
	// garbage-collect crypto verification tasks that the player no longer cares about
	d.crypto.ClearRoundsBefore(extSignals.currentRound)
	...
	for _, extSignal := range extSignals.signals {
		if proto, err := d.ledger.ConsensusVersion(paramsRoundBranch(extSignal.CurrentRound)); err != nil {
			...
			// this might happen during catchup, since the Ledger.Wait fires
			// as soon as a new block is received by the ledger, which could be
			// far before it's being committed. In these cases, it should be
			// safe to ignore fast recovery; we will advance the ledger soon.
		} else {
			if config.Consensus[proto].FastPartitionRecovery {
