...
func makeTestLedgerWithConsensusVersion(state map[basics.Address]basics.BalanceRecord, consensusVersion func(basics.Round) (protocol.ConsensusVersion, error)) Ledger {
	l := new(testLedger)
	l.entries = make(map[basics.Round]bookkeeping.Block)
	l.certs = make(map[basics.Round]Certificate)
	l.nextRound = 1
