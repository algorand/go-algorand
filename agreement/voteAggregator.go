...
func (agg *voteAggregator) handle(r routerHandle, pr player, em event) (res event) {
	...
	case votePresent:
		...
		err := agg.filterVote(e.Proto, pr, r, uv, e.FreshnessData)
		if err != nil {
	...
	case voteVerified:
		...
		v := e.Input.Vote
		err := agg.filterVote(e.Proto, pr, r, v.u(), e.FreshnessData)
		if err != nil {
			return filteredEvent{T: voteFiltered, Err: makeSerErr(err)}
		}
		...
		deliver := voteAcceptedEvent{Vote: v, Proto: e.Proto}
	...
	case bundleVerified:
		...
		for _, vote := range votes {
			deliver := voteAcceptedEvent{Vote: vote, Proto: e.Proto}
