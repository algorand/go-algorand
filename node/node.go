...
recreateNetwork:
	if cfg.EnableP2PHybridMode {
		p2pNode, err = network.NewHybridP2PNetwork(node.log, node.config, rootDir, phonebookAddresses, genesisInfo, node, &network.HybridRelayMeshStrategyCreator{})
		if err != nil {
		...
		p2pNode, err = network.NewP2PNetwork(node.log, node.config, rootDir, phonebookAddresses, genesisInfo, node, nil, &network.BaseMeshStrategyCreator{})
		if err != nil {
		...
		wsNode, err = network.NewWebsocketNetwork(node.log, node.config, phonebookAddresses, genesisInfo, node, nil, &network.BaseMeshStrategyCreator{})
		if err != nil {
