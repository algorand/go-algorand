...
func TestStoringKeys(t *testing.T) {
	...
	s := generateTestSigner(crypto.ParalithiumType, 0, 4096, 345, a)
	k := s.keyStore
	defer k.store.Close()
...
func TestDroppingKeys(t *testing.T) {
	...
	s := generateTestSigner(crypto.ParalithiumType, 25, 1023, 23, a)
	k := s.keyStore
	defer k.store.Close()
...
func BenchmarkFetchKeys(b *testing.B) {
	...
	interval := uint64(128)
	s := generateTestSigner(crypto.ParalithiumType, start, end, interval, a)
