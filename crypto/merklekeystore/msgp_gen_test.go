...
func TestMarshalUnmarshalEphemeralPublicKey(t *testing.T) {
	v := EphemeralPublicKey{}
	bts := v.MarshalMsg(nil)
	left, err := v.UnmarshalMsg(bts)
	if err != nil {
		t.Fatal(err)
...
func TestRandomizedEncodingEphemeralPublicKey(t *testing.T) {
	protocol.RunEncodingTest(t, &EphemeralPublicKey{})
}

func BenchmarkMarshalMsgEphemeralPublicKey(b *testing.B) {
	v := EphemeralPublicKey{}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkAppendMsgEphemeralPublicKey(b *testing.B) {
	v := EphemeralPublicKey{}
	bts := make([]byte, 0, v.Msgsize())
	bts = v.MarshalMsg(bts[0:0])
	b.SetBytes(int64(len(bts)))
...
func BenchmarkUnmarshalEphemeralPublicKey(b *testing.B) {
	v := EphemeralPublicKey{}
	bts := v.MarshalMsg(nil)
	b.ReportAllocs()
	b.SetBytes(int64(len(bts)))
