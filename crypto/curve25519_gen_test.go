...
func TestMarshalUnmarshal_Ctype_void(t *testing.T) {
	v := _Ctype_void{}
	bts, err := v.MarshalMsg(nil)
	if err != nil {
		t.Fatal(err)
...
func BenchmarkMarshalMsg_Ctype_void(b *testing.B) {
	v := _Ctype_void{}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkAppendMsg_Ctype_void(b *testing.B) {
	v := _Ctype_void{}
	bts := make([]byte, 0, v.Msgsize())
	bts, _ = v.MarshalMsg(bts[0:0])
	b.SetBytes(int64(len(bts)))
...
func BenchmarkUnmarshal_Ctype_void(b *testing.B) {
	v := _Ctype_void{}
	bts, _ := v.MarshalMsg(nil)
	b.ReportAllocs()
	b.SetBytes(int64(len(bts)))
