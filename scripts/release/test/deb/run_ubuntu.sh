...
sg docker "docker run --rm --env-file ${HOME}/build_env_docker --mount type=bind,src=${HOME}/keys,dst=/root/keys --mount type=bind,src=${HOME}/go,dst=/root/go --mount type=bind,src=/usr/local/go,dst=/usr/local/go ubuntu:16.04 bash /root/go/src/github.com/algorand/go-algorand/scripts/release/test/deb/test_algorand.sh"
...
sg docker "docker run --rm --env-file ${HOME}/build_env_docker \
  ...
  debian:stable \
  bash /root/go/src/github.com/algorand/go-algorand/scripts/release/test/deb/test_algorand.sh
"
