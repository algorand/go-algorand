...
rm -rf node_pkg/* && mkdir -p node_pkg/"$FULLVERSION"
ssh -i ReleaseBuildInstanceKey.pem -A ubuntu@"$INSTANCE" bash ben-branch/scripts/release/ci/upload.sh
scp -i ReleaseBuildInstanceKey.pem -o StrictHostKeyChecking=no -r ubuntu@"$INSTANCE":~/node_pkg/* node_pkg/"$FULLVERSION"/
aws s3 sync --exclude dev* --exclude master* --exclude nightly* --exclude stable* --acl public-read node_pkg/"$FULLVERSION" "$BUCKET_LOCATION"/"$CHANNEL"/"$FULLVERSION"_"$FULLVERSION"/
