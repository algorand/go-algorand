...
sg docker "docker run --rm --env-file ${HOME}/build_env_docker --mount type=bind,src=${HOME}/docker_test_resources,dst=/root/stuff --mount type=bind,src=${HOME}/go,dst=/root/go --mount type=bind,src=/usr/local/go,dst=/usr/local/go ubuntu:16.04 bash ${GOPATH}/src/github.com/algorand/go-algorand/scripts/release/helper/build_release_ubuntu_test_docker.sh"
