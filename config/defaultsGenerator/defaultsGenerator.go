...
const relayLocalVariableDeclaration = "var relayLocal"
...
var outputType = flag.String("t", "", "Type of the config")
...
func main() {
	...
	flag.Parse()
	if *outputfilename == "" || *packageName == "" || *headerFileName == "" || *jsonExampleFileName == "" || *outputType == "" {
	...
	var autoDefaultsBytes []byte
	switch strings.ToLower(*outputType) {
	case "relay":
		autoDefaultsBytes = []byte(prettyPrintRelay(config.AutogenRelay, "go"))
	default:
		autoDefaultsBytes = []byte(prettyPrintLocal(config.AutogenLocal, "go"))
	}
	...
	switch strings.ToLower(*outputType) {
	case "relay":
		autoDefaultsBytes = []byte(prettyPrintRelay(config.AutogenRelay, "json"))
	default:
		autoDefaultsBytes = []byte(prettyPrintLocal(config.AutogenLocal, "json"))
	}
...
func fieldsFor(t reflect.Type, fields map[fieldType]string, names byFieldName) (map[fieldType]string, byFieldName) {
	for fieldNum := 0; fieldNum < t.NumField(); fieldNum++ {
		field := t.Field(fieldNum)
		key := fieldType{
			Name: field.Name,
			Type: field.Type,
		}
...
func getFieldsLocal(c config.Local, format string) (map[fieldType]string, byFieldName) {
	fields := map[fieldType]string{}
	names := byFieldName{}
	fields, names = fieldsFor(reflect.TypeOf(c), fields, names)
	sort.Sort(names)
	fields = fillFieldsLocal(c, format, names, fields)
	return fields, names
}

func getFieldsRelay(c config.Relay, format string) map[fieldType]string {
	fields := map[fieldType]string{}
	names := byFieldName{}
	fields, names = fieldsFor(reflect.TypeOf(c), fields, names)
	sort.Sort(names)
	fields = fillFieldsRelay(c, format, names, fields)
	return fields
}

func fillFieldsLocal(c config.Local, format string, order byFieldName, fields map[fieldType]string) map[fieldType]string {
	for _, field := range order {
		switch field.Type.Kind() {
		case reflect.Bool:
			v := reflect.ValueOf(&c).Elem().FieldByName(field.Name).Bool()
			if format == "go" {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("\t%s:\t%v,\n", field.Name, v)
			} else {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("    \"%s\": %v,\n", field.Name, v)
			}
		case reflect.Int32:
		...
		case reflect.Int64:
			...
			if format == "go" {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("\t%s:\t%d,\n", field.Name, v)
			} else {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("    \"%s\": %d,\n", field.Name, v)
			}
		case reflect.Uint32:
		...
		case reflect.String:
			...
			if format == "go" {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("\t%s:\t\"%s\",\n", field.Name, v)
			} else {
				fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("    \"%s\": \"%s\",\n", field.Name, v)
			}
		case reflect.Map:
			if reflect.ValueOf(&c).Elem().FieldByName(field.Name).Len() == 0 {
				if format == "go" {
					...
					fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("\t%s:\tmap[%s]%s{},\n", field.Name, mapKeysType, mapValueType)
				} else {
					fields[fieldType{Name: field.Name, Type: field.Type}] = fmt.Sprintf("    \"%s\": {},\n", field.Name)
				}
			} else {
...
func fillFieldsRelay(c config.Relay, format string, order byFieldName, fields map[fieldType]string) map[fieldType]string {
	for _, field := range order {
		switch field.Type.Kind() {
		case reflect.Bool:
			v := reflect.ValueOf(&c).Elem().FieldByName(field.Name).Bool()
...
func prettyPrintLocal(c config.Local, format string) (out string) {
	fields, names := getFieldsLocal(c, format)
...
func prettyPrintRelay(c config.Relay, format string) (out string) {
	localFields, names := getFieldsLocal(config.AutogenLocal, format)
	relayFields := getFieldsRelay(c, format)

	if format == "go" {
		out = fmt.Sprintf("%s = Local{\n", relayLocalVariableDeclaration)
	} else {
		out = "{\n"
	}
	...
	for _, fName := range names {
		...
		if relayValue, ok := relayFields[key]; ok {
			out = out + relayValue
		} else {
			out = out + localFields[key]
		}
	}
