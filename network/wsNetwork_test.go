...
func TestWebsocketNetworkBasic(t *testing.T) {
	...
	t.Log(addrA)
	netB.phonebook = &oneEntryPhonebook{addr: addrA}
	netB.Start()
...
func lineNetwork(t *testing.T, numNodes int) (nodes []*WebsocketNetwork, counters []messageCounterHandler) {
	...
	for i := range nodes {
		...
		if i > 0 {
			...
			require.True(t, postListen)
			nodes[i].phonebook = &oneEntryPhonebook{addr: addrPrev}
...
func TestDupFilter(t *testing.T) {
	...
	netC.phonebook = &oneEntryPhonebook{addr: addrB}
	netC.Start()
	defer netC.Stop()
...
func TestWebsocketNetworkPrioLimit(t *testing.T) {
	...
	netC.config.GossipFanout = 1
	netC.phonebook = &oneEntryPhonebook{addr: addrA}
...
func TestWebsocketNetworkManyIdle(t *testing.T) {
	...
	for i := 0; i < numClients; i++ {
		...
		client.phonebook = &oneEntryPhonebook{addr: relayAddr}
		client.Start()
		defer client.Stop()
