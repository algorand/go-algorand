...
// TestPubsubTracer_TagList makes sure all entries
func TestPubsubTracer_TagList(t *testing.T) {
	t.Parallel()
	partitiontest.PartitionTest(t)

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "pubsubTracer.go", nil, 0)
	...
	require.Equal(t, len(TracedNetworkMessageTags), sendCaseCount)
	require.Equal(t, len(TracedNetworkMessageTags), recvCaseCount)
}
