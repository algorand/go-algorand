...
// minimalTransactionEncodedSize is an estimation for the encoded size of the transcation. It used only for
// estimating the number of transaction ids that would be generated by the selectPendingTransactions function.
const minimalTransactionEncodedSize = 64
...
func makePeer(networkPeer interface{}, isOutgoing bool, isLocalNodeRelay bool) *Peer {
	p := &Peer{
		...
		incomingMessages:       makeMessageOrderingHeap(),
	}
	if isOutgoing {
...
func (p *Peer) selectPendingTransactions(pendingTransactions []transactions.SignedTxGroup, sendWindow time.Duration, round basics.Round, bloomFilterSize int) (selectedTxns []transactions.SignedTxGroup, selectedTxnIDs []transactions.Txid, partialTranscationsSet bool) {
	...
	if windowLengthBytes < 0 {
		windowLengthBytes = 0
	}

	accumulatedSize := 0

	selectedTxnIDs = make([]transactions.Txid, 0, 1+windowLengthBytes/minimalTransactionEncodedSize)
